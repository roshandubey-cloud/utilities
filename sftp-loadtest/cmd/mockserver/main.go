// Mock SFTP server for end-to-end load tests.
//
...
//
// Routing pairs are passed via --pairs "up1=dl1,up2=dl2,...". Users without a
// pair route to their own outbox (self-loop), which is still a valid test.
//
// Uploads discard content but record size. Reads return zero-filled bytes of
// the recorded size so download throughput is measurable.
package main
...
func main() {
	...
	fs := newMockFS(*delay, parsePairs(*pairs))
	fs.failUsers = parseFailUsers(*failUsers)

	l, err := net.Listen("tcp", *addr)
	if err != nil {
		log.Fatalf("listen: %v", err)
	}
	log.Printf("mock sftp listening on %s  delay=%s  pairs=%v", l.Addr(), *delay, fs.routes)
...
func handleConn(nConn net.Conn, cfg *ssh.ServerConfig, fs *mockFS) {
	defer nConn.Close()
